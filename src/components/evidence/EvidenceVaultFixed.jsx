...
import { Camera, Mic, FileText, Lock, X, Check, Download, Trash2, AlertTriangle, Brain, Sparkles, Square, Clock } from 'lucide-react';
...
const EvidenceVaultFixed = ({ navigateTo, userData, setUserData, mlHealthy }) => {
  ...
  // FIX: isRecording state now properly toggled
  ...
  useEffect(() => {
    ...
    // FIX: Don't load audio from localStorage — blob URLs are session-only
    // Audio recordings are session-only (can't persist blob URLs across sessions)
    ...
    if (savedPhotos) {
      try { setPhotos(JSON.parse(savedPhotos)); } catch(e) {}
    }
    if (savedJournal) {
      try { setJournalEntries(JSON.parse(savedJournal)); } catch(e) {}
    }
  }, []);
  ...
  // FIX: startRecording now sets isRecording=true and attaches onstop handler inline
  const startRecording = async () => {
    try {
      ...
      // FIX: onstop handler defined here, with access to current mlHealthy
      mediaRecorder.onstop = async () => {
        ...
        // Note: blob URLs are session-only, not persisted to localStorage
      };

      mediaRecorder.start(100); // collect data every 100ms
      setIsRecording(true); // FIX: was missing — this is why stop button never showed
    } catch (error) {
  ...
  // FIX: stopRecording checks mediaRecorder state properly
  const stopRecording = () => {
      ...
      try {
        ...
        if (analysis.success) {
          alert(
            ...
            `• Photos: ${analysis.by_type?.photos || photos.length}\n` +
            `• Audio: ${analysis.by_type?.audio || audioRecordings.length}\n` +
            `• Journal: ${analysis.by_type?.journal || journalEntries.length}\n\n` +
            `Overall Strength: ${analysis.overall_strength || 'N/A'}\n` +
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-orange-50 to-red-50 p-4 flex items-center justify-center">
        <div className="max-w-md w-full bg-white rounded-2xl p-8 shadow-2xl">
          ...
          <button onClick={handleSetupPin} disabled={pin.length < 4 || pin !== confirmPin}
            className="w-full bg-gradient-to-r from-orange-600 to-orange-700 text-white py-4 rounded-xl font-bold shadow-lg disabled:opacity-50 mb-3">
            Create PIN
          </button>
          <button onClick={() => navigateTo('landing')}
            className="w-full bg-gray-100 text-gray-700 py-3 rounded-xl font-medium">Cancel</button>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="fixed inset-0 bg-black z-50 flex flex-col">
        ...
        <div className="bg-black p-6 flex justify-around items-center">
          <button onClick={stopCamera} className="bg-gray-700 hover:bg-gray-600 text-white p-4 rounded-full">
            <X className="h-6 w-6" />
          </button>
          <button onClick={capturePhoto} className="bg-white hover:bg-gray-100 p-6 rounded-full shadow-2xl">
        ...
        <div className="max-w-2xl mx-auto">
          <div className="bg-white rounded-2xl p-6 shadow-lg mb-4">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setShowJournalForm(false)} className="text-gray-600 hover:text-gray-900">
                <X className="h-6 w-6" />
              </button>
            </div>
            {mlHealthy && (
            ...
            <input type="text" placeholder="Incident Title *" value={journalEntry.title}
            ...
            <textarea placeholder="Detailed Description * (What happened, who was involved, what was said...)"
            ...
            <div className="grid grid-cols-2 gap-3 mb-3">
              <input type="date" value={journalEntry.date}
              ...
              <input type="time" value={journalEntry.time}
                onChange={e => setJournalEntry({...journalEntry, time: e.target.value})}
                className="p-3 border-2 border-gray-200 rounded-xl focus:border-orange-400 focus:outline-none" />
            </div>
            <input type="text" placeholder="Location (optional)" value={journalEntry.location}
              onChange={e => setJournalEntry({...journalEntry, location: e.target.value})}
              className="w-full p-3 border-2 border-gray-200 rounded-xl mb-3 focus:border-orange-400 focus:outline-none" />
            <input type="text" placeholder="Witnesses (optional)" value={journalEntry.witnesses}
            ...
            <button onClick={saveJournalEntryWithML} disabled={analyzingJournal}
              className="w-full bg-gradient-to-r from-orange-600 to-orange-700 text-white py-4 rounded-xl font-bold shadow-lg disabled:opacity-50">
              {analyzingJournal ? (
                <span className="flex items-center justify-center space-x-2">
                  <Brain className="h-5 w-5 animate-pulse" /><span>Analyzing with AI...</span>
                </span>
        ...
        <div className="bg-white rounded-2xl p-6 mb-6 shadow-lg">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex space-x-3">
              ...
              <button onClick={() => { setUnlocked(false); setShowPinPrompt(true); setPin(''); }}
            ...
            <div>
              <div className="flex items-center justify-between mb-4">
                ...
                <button onClick={startCamera}
                  className="bg-gradient-to-r from-orange-600 to-orange-700 text-white px-6 py-3 rounded-xl font-semibold shadow-lg flex items-center space-x-2">
                    ...
                    <div key={photo.id} className="relative group">
                      ...
                      {photo.mlAnalysis && (
                        <span className="text-xs bg-purple-100 text-purple-700 px-2 py-0.5 rounded-full">
                          AI: {photo.mlAnalysis.evidence_quality}
                        </span>
                      )}
                    </div>
                  ))}
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                {!isRecording ? (
                  <button onClick={startRecording}
                    className="bg-gradient-to-r from-red-600 to-red-700 text-white px-6 py-3 rounded-xl font-semibold shadow-lg flex items-center space-x-2">
                    <Mic className="h-5 w-5" /><span>Start Recording</span>
                  </button>
                ) : (
                  <button onClick={stopRecording}
                    className="bg-gradient-to-r from-gray-800 to-gray-900 text-white px-6 py-3 rounded-xl font-semibold shadow-lg flex items-center space-x-3">
                      ...
                      <div className="flex items-start justify-between mb-2">
                        <div>
                          ...
                          {recording.duration > 0 && (
                            <p className="text-xs text-gray-500">Duration: {formatTime(recording.duration)}</p>
                          )}
                          {recording.mlAnalysis && (
                            <span className="text-xs bg-purple-100 text-purple-700 px-2 py-0.5 rounded-full mt-1 inline-block">
                              AI: {recording.mlAnalysis.evidence_quality || 'Analyzed'}
                            </span>
                          )}
                        </div>
                        <button onClick={() => deleteRecording(recording.id)} className="text-red-600 hover:text-red-700 p-1">
                          <Trash2 className="h-5 w-5" />
                        </button>
                      </div>
                  ...
                  {journalEntries.map(entry => (
                    <div key={entry.id} className="p-6 bg-gradient-to-r from-orange-50 to-pink-50 rounded-xl border border-orange-200">
                      <div className="flex items-start justify-between mb-3">
                        ...
                        <button onClick={() => deleteJournalEntry(entry.id)} className="text-red-600 hover:text-red-700">
                          <Trash2 className="h-5 w-5" />
                        </button>
                      </div>
